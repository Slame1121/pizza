...
class ControllerApiOrder extends Controller {
	...
	public function history() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($order_info) {
				if( $this->request->post['order_status_id'] == 5 || $this->request->post['order_status_id']== 3){
					//Если статус гуд, переводим зарезервированные бонусы юзеру
					$this->model_checkout_order->giveOrderBonuses($order_id);
				}
